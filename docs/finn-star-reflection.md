...
- distinguish between supported IMUs
- report which bus and address were active
...
On the host side, I added the Python recording script so the streamed data could be saved into CSV files with timestamps and labels. That made the pipeline reproducible instead of ad hoc.
...
That reflects a deliberate strategy: validate the pipeline on a smaller case first, then scale the dataset once collection and labeling are stable.
...
In other words, my work turned the project from a hardware idea into a usable data source.
...
- `f812c19` `feat: init commit`
  initial Rust firmware, monitor tooling, project skeleton
- `55ea57f` `refactor: used external imu`
  external IMU path and hardware integration improvements
- `eb0e732` `feat: added record py script`
  host-side recording pipeline
...
- `07d366d` `refactor: highlighted the event period in plotting`
  clearer event interpretation during analysis
