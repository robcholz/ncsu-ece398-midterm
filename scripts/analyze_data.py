...
def analyze_file(filepath):
    ...
    timestamps = [
        parse_numeric(row.get("timestamp", ""))
        for row in data
        if parse_numeric(row.get("timestamp", "")) is not None
    ]
    intervals = []
...
def generate_markdown_report(results, recordings_dir):
    ...
    report.append(
        f"- Sample count range: {min(sample_counts)} - {max(sample_counts)}\n"
    )
    ...
    for event_name in sorted(results.keys()):
        ...
        report.append(
            f"- Range: {min(event_sample_rates):.1f} - {max(event_sample_rates):.1f} Hz\n\n"
        )
        ...
        for f in files:
            all_acc_x.extend(
                [
                    parse_numeric(v)
                    for v in [f["acc_x"]["min"], f["acc_x"]["max"], f["acc_x"]["mean"]]
                ]
            )
            all_acc_y.extend(
                [
                    parse_numeric(v)
                    for v in [f["acc_y"]["min"], f["acc_y"]["max"], f["acc_y"]["mean"]]
                ]
            )
            all_acc_z.extend(
                [
                    parse_numeric(v)
                    for v in [f["acc_z"]["min"], f["acc_z"]["max"], f["acc_z"]["mean"]]
                ]
            )
        ...
        if all_acc_x:
            report.append(
                f"- Acc X: {min(all_acc_x):.3f} to {max(all_acc_x):.3f} (mean: {mean(all_acc_x):.3f})\n"
            )
        if all_acc_y:
            report.append(
                f"- Acc Y: {min(all_acc_y):.3f} to {max(all_acc_y):.3f} (mean: {mean(all_acc_y):.3f})\n"
            )
        if all_acc_z:
            report.append(
                f"- Acc Z: {min(all_acc_z):.3f} to {max(all_acc_z):.3f} (mean: {mean(all_acc_z):.3f})\n"
            )
        report.append("\n")
        ...
        if len(by_location) > 1:
            ...
            for location in sorted(by_location.keys()):
                ...
                report.append(
                    f"- **{location.title()}** ({len(location_files)} files): "
                )
        ...
        report.append(
            "| File | Subject | Location | Duration (s) | Samples | Sample Rate (Hz) |\n"
        )
        report.append(
            "|------|---------|----------|--------------|---------|------------------|\n"
        )
        ...
        for f in files:
            ...
            report.append(
                f"| {filename} | {f['subject'] or 'N/A'} | {f['location'] or 'N/A'} | "
            )
            report.append(
                f"{f['duration_sec']} | {f['sample_count']} | {f['sample_rate_hz']} |\n"
            )
